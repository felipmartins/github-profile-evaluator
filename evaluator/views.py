...
def pdf_export(request, type: str, uuid: str):
    ...
    for evaluation in evaluations:
        ...
        height += 45
        p.drawCentredString(A4[0]/2., A4[1]-height, 'Avaliação realizada em ' + evaluation.evaluation_date.strftime("%d/%m/%Y"))
        logo = ImageReader('evaluator/static/photos/'+ evaluation.github_user +'_image.jpg')
        logo.getSize()
        width = logo._width
        height += 235
        p.drawImage(logo, A4[0]/2-width/2, A4[1]-height, height=200, preserveAspectRatio=True ,mask = None)
        ...
        if evaluation.has_photo:
            pergunta1 = ['Rosto encontrado pelo algoritmo na foto de perfil?', 'Sim!', "Sim!", "+10 pontos"]
        else:
            pergunta1 = ['Rosto encontrado pelo algoritmo na foto de perfil?', 'Não', "Sim!", "0 pontos"]
        if evaluation.has_email:
            pergunta2 = ['E-mail localizado dentro do README?', 'Sim!', "Sim!", "+10 pontos"]
        else:
            pergunta2 = ['E-mail localizado dentro do README?', 'Não', "Sim!", "0 pontos"]
        if evaluation.has_linkedin:
            pergunta3 = ['LinkedIn localizado dentro do GitHub?', 'Sim!', "Sim!", "+10 pontos"]
        else:
            pergunta3 = ['LinkedIn localizado dentro do GitHub?', 'Não', "Sim!", "0 pontos"]
        if evaluation.stacks >= 5:
            pergunta4 = ['Tecnologias de domínio localizadas no perfil', evaluation.stacks, ">=5", "+10 pontos"]
        else:
            pergunta4 = ['Tecnologias de domínio localizadas no perfil', evaluation.stacks, ">=5", "0 pontos"]
        if evaluation.stacks >= 10:
            pergunta5 = ['Tecnologias de domínio localizadas no perfil', evaluation.stacks, ">=10", "+10 pontos"]
        else:
            pergunta5 = ['Tecnologias de domínio localizadas no perfil', evaluation.stacks, ">=10", "0 pontos"]
        if evaluation.repositories >= 5:
            pergunta6 = ['Repositórios públicos localizados no GitHub', evaluation.repositories, ">=5", "+10 pontos"]
        else:
            pergunta6 = ['Repositórios públicos localizados no GitHub', evaluation.repositories, ">=5", "0 pontos"]
        if evaluation.repositories >= 10:
            pergunta7 = ['Repositórios públicos localizados no GitHub', evaluation.repositories, ">=10", "+10 pontos"]
        else:
            pergunta7 = ['Repositórios públicos localizados no GitHub', evaluation.repositories, ">=10", "0 pontos"]
        if evaluation.pinned_repositories >= 2:
            pergunta8 = ['Repositórios pinados localizados no GitHub', evaluation.pinned_repositories, ">=5", "+10 pontos"]
        else:
            pergunta8 = ['Repositórios pinados localizados no GitHub', evaluation.pinned_repositories, ">=5", "0 pontos"]
        if evaluation.pinned_repositories >= 4:
            pergunta9 = ['Repositórios pinados localizados no GitHub', evaluation.pinned_repositories, ">=10", "+10 pontos"]
        else:
            pergunta9 = ['Repositórios pinados localizados no GitHub', evaluation.pinned_repositories, ">=10", "0 pontos"]
        ...
        f = Table(data, colWidths=[270,90,90,90], rowHeights=[25,25,25,25,25,25,25,25,25,25])
        f.setStyle(TableStyle([('INNERGRID', (0,0), (-1,-1), 0.25, colors.black),
                               ('BOX', (0,0), (-1,-1), 0.25, colors.black),
                               ('ALIGN',(1,0), (-1,-1),'CENTER'),
                               ('VALIGN',(0,0), (-1,-1),'MIDDLE'),
                               ('BACKGROUND',(0,0), (-1,0), colors.Color(red=(.1),green=(.1),blue=(.1))),
